...
const Step3SelectBots = ({ onNext, onBack, data }) => {
  ...
  const total = selected.reduce((sum, key) => {
    const bot = bots.find((b) => b.key === key);
      ...
      <Grid container spacing={2}>
        {bots.map((bot) => (
          <Grid item xs={12} sm={6} key={bot.key}>
            <Card variant={selected.includes(bot.key) ? 'elevation' : 'outlined'} sx={{ borderColor: selected.includes(bot.key) ? 'primary.main' : 'grey.300', boxShadow: selected.includes(bot.key) ? 4 : 0 }}>
              <CardContent>
                <FormControlLabel
